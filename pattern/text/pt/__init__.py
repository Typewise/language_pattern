...
####################################################################################################
# Template for pattern.pt, bundling natural language processing tools for language ptptX.
...
# Tools for word inflection should be bundled in pattern.text.pt.inflect.
...
# The lexicon for pattern.pt may be using a different tagset (e.g., PAROLE, WOTAN).
# The following functions are meant to map the tags to Penn Treebank II, see Parser.find_chunks().

TAGSET = {"??": "NN"} # pattern.pt tagset => Penn Treebank II.
...
# for pattern.pt, see also Parser.find_tokens().

REPLACEMENTS = {"'s": " 's", "'ve": " 've"}
ABBREVIATIONS = set(("e.g.", "etc.", "i.e."))

# A lemmatizer can be constructed if we have a pattern.pt.inflect,
...
parser = Parser(
     lexicon = os.path.join(MODULE, "pt-lexicon.txt"),    # A dict of known words => most frequent tag.
   frequency = os.path.join(MODULE, "pt-frequency.txt"),  # A dict of word frequency.
  morphology = os.path.join(MODULE, "pt-morphology.txt"), # A set of suffix rules.
     context = os.path.join(MODULE, "pt-context.txt"),    # A set of contextual rules.
    entities = os.path.join(MODULE, "pt-entities.txt"),   # A dict of named entities: John = NNP-PERS.
     default = ("NN", "NNP", "CD"),
    language = "pt"
)
...
#---------------------------------------------------------------------------------------------------
# python -m pattern.pt xml -s "..." -OTCL
